...
class GridProcessor implements DataProcessorInterface
{
    ...
    protected function getTextColSizes()
    {
        ...
        $imgColSizeSm = ($this->getImgColSizeSm() > 0) ? $this->getImgColSizeSm() : 12;
        $imgColSizeMd = ($this->getImgColSizeMd() > 0) ? $this->getImgColSizeMd() : 12;
        $imgColSizeLg = ($this->getImgColSizeLg() > 0) ? $this->getImgColSizeLg() : 12;

        $textColumnSizeXs = (int)$this->getProcessorConfiguration()['bootstrap_grid_size'] - $imgColSizeXs;
        $textColumnSizeSm = (int)$this->getProcessorConfiguration()['bootstrap_grid_size'] - $imgColSizeSm;
        $textColumnSizeMd = (int)$this->getProcessorConfiguration()['bootstrap_grid_size'] - $imgColSizeMd;
        $textColumnSizeLg = (int)$this->getProcessorConfiguration()['bootstrap_grid_size'] - $imgColSizeLg;


        $textColSizeArr = [
            'xs' => ($textColumnSizeXs ? 'col-xs-' . $textColumnSizeXs : 'col-xs-12'),
            'sm' => ($textColumnSizeSm ? 'col-sm-' . $textColumnSizeSm : 'col-sm-12'),
            'md' => ($textColumnSizeMd ? 'col-md-' . $textColumnSizeMd : 'col-md-12'),
            'lg' => ($textColumnSizeLg ? 'col-lg-' . $textColumnSizeLg : 'col-lg-12')
        ];
